...
function SandTrap() {
    ...
    this.trap = 2; // Player starts on Trap Level 2 to begin.
...
SandTrap.trapLevel = function(amount) {
    ...
    if (amount != 0) {
        monster.trap = monster.trap + amount;
        if (monster.trap < 1) monster.trap = 1;
        if (monster.trap > 4) monster.trap = 4;
    }
    return monster.trap;
};
...
//TODO spriteSelect(97);
    ...
    if (player.findStatusEffect(StatusEffects.Climbed) < 0) player.createStatusEffect(StatusEffects.Climbed, 0, 0, 0, 0);
...
SandTrap.prototype.doAI = function() {
    ...
    if (monster.trap == 4 && player.findStatusEffect(StatusEffects.Climbed) < 0) SandTrap.grabAttack();
    ...
    if (player.findStatusEffect(StatusEffects.Climbed) < 0) {
    ...
    else player.removeStatusEffect(StatusEffects.Climbed);
    // Is the player at the lowest level? Auto loss.
    if (monster.trap = 1) SandTrapScene.sandTrapLoss();
    //Otherwise, continue
    combatRoundOver();
};
