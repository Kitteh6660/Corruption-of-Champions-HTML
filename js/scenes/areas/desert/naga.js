...
function Naga() {
    ...
    //Victory/defeat
    this.victory = NagaScene.nagaWin;
    this.defeat = NagaScene.nagaLoss;
}
...
Naga.prototype.doAI = function() {
    switch(rand(4)) {
        case 0:
            if (this.venom >= 25) { // With this check, will the player ever see the out of venom message in the attack?
...
// Naga Poison Bite Attack, See note in doAI.
Naga.nagaPoisonBiteAttack = function() {
    ...
    if (player.findStatusEffect(StatusEffects.Venom) < 0) {
        ...
        if (player.spe > 3) {
            player.spe -= 2;
            player.addStatusValue(StatusEffects.Venom, 1, 2);
        }
        else
            damage += 5 + rand(5);
        damage += 5 + rand(5);
...
// Naga Constriction Attack NEEDS PERK AND ARMOR PERK STUFF!
...
NagaScene.nagaWin = function() {
    NagaScene.nagaRapeChoice();
};

NagaScene.nagaLoss = function() {
    if (player.findStatusEffect(StatusEffects.Infested) >= 0) {
        outputText("<br><br>The naga's eyes go wide and she turns to leave, no longer interested in you.");
        //player.orgasm(); Player orgasming after defeat with worms makes no sense? Maybe do a check to see if they lost by lust first?
        cleanupAfterCombat();
        return;
    }
    else {
        NagaScene.nagaRapesPlayer();
    }
};

// WIN SCENES

NagaScene.nagaRapeChoice = function() {
    ...
    var eggs = null;
    if (player.canOvipositSpider()) eggs = NagaScene.eggUpANagaSpiderLike;
    if (player.canOvipositBee() && player.gender > 0) eggs = NagaScene.beePositANagaPlease;
    if (player.lust >= 33 && gameFlags[SFW_MODE] <= 0) {
        outputText("Your body aches for further satisfaction - do you rape the snake woman?");
        menu();
        if (player.gender == 0) addButton(0, "Yes", NagaScene.nagaVictoryGenderless, null, null, null, "TO BE ADDED.");
        if (player.gender == 1) addButton(0, "Yes", NagaScene.nagaVictoryMale, null, null, null, "TO BE ADDED.");
        if (player.gender == 2) addButton(0, "Yes", NagaScene.nagaVictoryFemale, null, null, null, "TO BE ADDED.");
        if (player.gender == 3) {
        ...
        if (eggs != null)
            addButton(3, "Eggs", eggs, null, null, null, "TO BE ADDED.");
        addButton(4, "Leave", cleanupAfterCombat, null, null, null, "TO BE ADDED");
        return;
        }
    cleanupAfterCombat();
};
...
// Genderless win
NagaScene.nagaVictoryGenderless = function() {
    clearOutput();
    //c)Centaur
    if (player.isTaur()) {
        outputText("You prance over to the prone form of the snake woman, plotting just what you will do to her body before realizing the severe lack of parts to use on her. You let loose a snort of frustration and paw at the ground. This complicates things, but you aren't going to let a little thing like having no genitalia stop you from having some fun.<br><br>");
        outputText("You position yourself over the uppermost part of her tail and lower yourself down onto her, the lower half of your body resting on her tail. You reach down to the scaly covering over her pussy and open it to reveal the soft folds inside. Gently you slide your tongue into her, making small circles every so often.<br><br>");
        outputText("A hiss of pleasure escapes from the naga's mouth as she brings her hands onto your head and grips you tightly. The lower portion of her tail wraps gently around you and she brings the tip of her tail up to your mouth. You shift your attention from her netherlips to start sucking on the tip of it, twirling your tongue around it sensually. After a moment, the naga takes her tongue out of your mouth and slides the now lubricated tail gently into your backside. You let out a slight gasp, but it is cut short as the naga brings your head back down to her pussy. You start licking again as the naga slides more of her tail into you, twitching the tip of it deep inside of you.<br><br>");
        outputText("You speed up your licking and thrust your tongue deeper into her, her walls now slick. She presses your face deep into her pussy as her hips buck in orgasm.<br><br>");
        outputText("The tail of the naga slides out of you as she lies back in the sand of the desert, exhausted from her orgasm. You stand up and gather your things, thinking that maybe it wouldn't be too bad of an idea to come back.<br><br>");
    }
    //a)Uncorrupted
    else if (player.cor < 50) {
        outputText("You look down at the collapsed naga, feeling the blood pounding hotly through your veins. You stand above her fallen form and start to take off your pants when you remember that you have nothing down there. A soft curse escapes your lips; this will make things much more difficult, but there WILL be sex one way or another.<br><br>");
        outputText("You straddle the naga, placing your bare, featureless crotch against the slit of her vagina. Slowly you rock your hips back and forth against her, drawing out a gasp of pleasure from her. Leaning over her you start to massage her breasts.<br><br>");
        ...
        outputText("You stand up over her, your crotch now wet from her girl cum. You pick up your things and put your clothes back on, leaving the naga lying there in bliss.<br><br>");
    }
    //b)Corrupted
    else {
...
// Male win
NagaScene.nagaVictoryMale = function() {
    clearOutput();
    //player.orgasm();
    //Male or 50% herms
    if (player.totalCocks() > 0) {
        ...
        //Corrupted (event raises fatigue by 20)
            ...
            outputText(".<br><br>");
            outputText("As you retrieve your clothes, you notice red marks around your chest and arms. It seems that while the naga did have the opportunity to kill you while you were unconscious, she preferred to keep you alive.<br><br>");
        }
    }
    outputText("You think it would be a very good idea to come to the desert more often.");
    cleanupAfterCombat();
...
// Female win
NagaScene.nagaVictoryFemale = function() {
    clearOutput();
    //4) Victory female
    if (player.hasVagina()) {
        //c) Centaur
        ...
        //b) Corrupted
            ...
            outputText("<br><br>You return to camp, satisfied with your actions.<br><br>");
        }
    }
    outputText("You think it would be a very good idea to come to the desert more often.");
   cleanupAfterCombat();
...
// Goo win
NagaScene.gooNagaRape = function() {
    clearOutput();
    ...
    outputText("  You slide your gooey form over her body, resting at her crotch. She tries to push you off, but you easily resist her.<br><br>");
    ...
    outputText("<br><br>You continue to relentlessly push yourself into the naga, stretching her wide.  ");
    //(if male)
    if (player.gender == 1) {
    ...
    //(if female)
        ...
        //(if squirter)
        if (player.vaginas[0].vaginalWetness == VAGINA_WETNESS_SLAVERING) outputText(", your girlcum gushing out and pooling on the stomach of the naga");
        outputText(".");
    }
    //(if herm)
    if (player.gender == 3) {
...
// Spider egg win
NagaScene.eggUpANagaSpiderLike = function() {
    clearOutput();
    ...
    outputText("<br><br>A while later, you awaken alone in the desert sand.  Getting your shaky legs under you and stifling a yawn, you head back to camp.  You should come to the desert more often.");
    player.dumpEggs();
    cleanupAfterCombat();
    player.orgasm();
};
...
// Bee egg win
NagaScene.beePositANagaPlease = function() {
    clearOutput();
    ...
    outputText("<br><br>Panting softly, the confused, climax-addled snake weakly kisses you again, though it falls apart once you withdraw your spent egg-tube from the gushing, honey-scented snatch.  A small river of amber goo runs from her slightly-gaped pussy as you withdraw, though your eggs are pushed so deep as to be invisible");
    ...
    outputText("<br><br>You should definitely come to the desert more often.");
    player.dumpEggs();
    cleanupAfterCombat();
    player.orgasm();
};
...
NagaScene.nagaRapesPlayer = function() {
    clearOutput();
    ...
    if ((player.findPerk(PerkLib.BimboBrains) >= 0 || player.findPerk(PerkLib.FutaFaculties) >= 0) && player.hasVagina()) {
        ...
        //[If Lactating]
        if (player.lactationQ() >= 50 && player.biggestLactation() >= 1) outputText("Your warm milk is leaking from your " + player.nippleDescript(0) + "s the entire time, coating your skin and soaking her hands. At one point, she withdraws a hand and licks the milk from it. You can only assume that she liked it, as she begins to focus even more on your breasts.<br><br>");
        //-------

        outputText("The naga lifts you from her body with her tail, placing you onto the warm sand where she had been laying. She moves her own body above yours, her tail buried deep inside your " + player.vaginaDescript(0) + " the entire time. She holds you in place with one hand, her other hand moving to resume playing with your " + player.allBreastsDescript() + ". She moves her head in, her hot breath washing over your neck, and looks up into your eyes for a moment. She smiles, giggles, and then wraps her mouth around your " + player.nippleDescript(0) + ", letting her slender tongue tickle your sensitive areola.<br><br>");
        //-----

        //[If lactating]
        if (player.lactationQ() >= 50 && player.biggestLactation() >= 1) outputText("As her mouth sucks and plays with your nipple, your warm, sweet milk begins to flow forth once again, filling her mouth with your delectable cream. Her hand is also absolutely covered by it, and the excess drips down to the dry sand below, absorbed in seconds by the thirsty earth.<br><br>");
        //------

        outputText("All this time, her tail has been working its way slowly inside you, stretching you farther with every thrust. As you open your mouth wide to let out another passionate wail, she moves in, pushing her lips over yours and kissing you passionately.<br><br>");

        //----
        //[If Lactating]
        if (player.lactationQ() >= 50 && player.biggestLactation() >= 1) outputText("She shares a mouthful of your delicious milk with you.<br><br>");
        //------
    ...
    //too)
        ...
        //(If defeated by lust)
        if (player.lust >= player.maxLust()) outputText("You fall to your knees, your body shaking with desire.<br><br>");
        //(If defeated by KO)
        else outputText("You fall to your knees, your battered body unable to hold itself up any longer.<br><br>");
        ...
        outputText("  She leans towards you, gripping your shoulders as she brings her face to your ear. You can feel her serpentine tongue flicking over your ear as she hisses coldly into it.<br><br>");
    ...
    //b) female //http://nekomimichan.org/mg/src/12844585127.jpg
        ...
        {
            outputText("You fall to your knees, your body shaking with desire.<br><br>");
        }
        //If defeated by KO)
        else
        {
            outputText("You fall to your knees, your battered body unable to hold itself up any longer.<br><br>");
        }
        ...
        outputText("Eventually, it appears that the naga is satisfied, and she stops, nudging you to do so as well.<br><br>");

        outputText("Then the naga lies flat on the sandy ground, pulling your body down onto on hers, face up, with your head resting between her soft breasts. She reaches around your body, placing her hands on your own " + player.chestDesc() + " gently holding you in place as her tail moves closer to your " + player.vaginaDescript(0) + ". As she begins to rub her presently-slick tail against your slit, the scaly tip brushes your " + player.clitDescript() + " and you moan with passion in response.  There is no holding it back; this feeling is one of the best you have experienced. Her tail continues to flick and rub your " + player.clitDescript() + " as you begin to buck your hips gently. The naga pokes and rubs at your " + player.clitDescript() + " for some time, eventually driving you to the first of many coming orgasms. After your breath has become even again and your muscles have become relaxed, the naga resumes her advances. She begins to rub the tip of her tail up and down along your " + player.vaginaDescript(0) + ", and you finally understand her intentions.<br><br>");

        outputText("As the tail begins to penetrate you, you cannot help but let out another passionate moan. The scaly skin rubs at your tender vaginal walls like nothing else you have experienced, and the feeling begins to drive you mad with pleasure. It only takes a few minutes of this gentle tail-fucking to drive you to another climax. This time, however, the naga doesn't wait for your recovery. Instead, she responds by plunging deeper, forcing her ever-thickening tail further into your " + player.vaginaDescript(0) + ". As she stuffs more and more of her scaly tail inside you, her hands begin to work on your " + player.chestDesc() + ", gently caressing and massaging them.");
        //(If fuckable nipples)
        if (player.hasFuckableNipples()) {
            outputText("  As her hands rub across your " + player.chestDesc() + ", the naga takes notice of the size of your nipples. She begins to press on them with her fingers, and, to her surprise, her fingers begin to disappear inside your breasts. It doesn't take her long to figure it out, and before you know it she is finger-fucking both your " + player.nippleDescript(0) + " in sync with the thrusts of her tail.");
        }
        //(If lactating)
        ...
        outputText("The naga lifts you from her body with her tail, placing you onto the warm sand where she had been laying. She moves her own body above yours, her tail buried deep inside your " + player.vaginaDescript(0) + " the entire time. She holds you in place with one hand, her other hand moving to resume playing with your " + player.chestDesc() + ". She moves her head in, her hot breath washing over your other breast, and looks up into your eyes for a moment. She smiles, giggles, and then wraps her mouth around your " + player.nippleDescript(0) + ", letting her slender tongue tickle your sensitive skin there.");
        //(If fuckable nipples)
        if (player.hasFuckableNipples()) {
