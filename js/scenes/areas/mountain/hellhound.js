...
Hellhound.hellhoundFire = function() {
    ...
    /*if (player.hasStatusAffect(StatusAffects.Web_dash_Silence) >= 0) {
     outputText("You reach inside yourself to breathe flames, but as you ready to release a torrent of fire, it backs up in your throat, blocked by the webbing across your mouth.  It causes you to cry out as the sudden, heated force explodes in your own throat.<br>", false);
     changeFatigue(10);
    ...
    if (player.findPerk(PerkLib.Evade) >= 0 && player.spe >= 35 && rand(3) != 0) {
        ...
        outputText("Using Raphael's teachings and the movement afforded by your bodysuit, you anticipate and sidestep " + monster.a + monster.refName + "'s fire.<br>", false);
    }
...
HellhoundScene.hellhoundWin = function() {
    ...
    if (monster.HP <= 0) {
        ...
        if (player.hasVagina() && player.lust >= 33 && !player.isNaga()) {
            ...
            addButton(0, "Fuck It", HellhoundScene.hellhoundPropahRape);
        ...
        outputText("Unable to bear hurting you anymore, the hellhound's flames dim as he stops his attack. The two heads look at you, whining plaintively.  The hellhound slowly pads over to you and nudges its noses at your crotch.  It seems he wishes to pleasure you.<br><br>", true);
        var temp2 = null;
        if (player.gender > 0 && player.lust >= 33) {
            ...
            addButton(0, "Lick", HellhoundScene.hellhoundGetsRaped);
            addButton(1, "Fuck", HellhoundScene.hellhoundPropahRape);
            addButton(5, "Leave", cleanupAfterCombat);
        }
        else {
...
//TODO Worms
HellhoundScene.hellhoundLoss = function() {
    /*
    if(pcCameWorms){
        outputText("<br><br>The hellhound snorts and leaves you to your fate.", false);
        doNext(game.combat.cleanupAfterCombat);
    } else { */
    HellhoundScene.hellhoundRapesPlayer();
};

HellhoundScene.hellhoundRapesPlayer = function() {
    if (gameFlags[SFW_MODE] == 1) return; //No rape in SFW mode.
//Lust or HP loss texts here
    if (player.lust >= player.maxLust()) outputText("Overcome by sexual desire, you submit to the hungry hellhound.<br><br>", true);
    else outputText("Too weak to continue fighting, you fall to your knees.<br><br>", true);
...
HellhoundScene.hellhoundGetsRaped = function() {
...
//[if player has no endowments]
    if (player.gender == 0) {
        outputText("The two heads stare at your lack of sexual endowments for a few seconds before looking up at you sadly.  With a whine, the hellhound wanders off.  Feeling rather unsatisfied, you think that you should probably fix your lack of sexual 'parts'...", false);
        cleanupAfterCombat();
    }
    else {
        player.orgasm();
...
{
    clearOutput();
outputText("You once again allow the master's will to fill you and bring you to his home.  Once again you pass through the many crags and passes of the mountains and find yourself in the hellhound valley.  The place is just as it was the last time you were here, but you don't take in the views, you have other pressing matters to deal with.  You quickly make your way across the valley to the entrance to the master's lair.<br><br>", false);

outputText("Another succubus stands guard at the entrance with another hellhound standing next to her.  She gives you a bit of a disdainful look before her hound steps forward.  \"<i>My mistress doesn't like dealing with lower hounds.  The grand master will be glad to see you, however.  Please go right in.</i>\"  One of its heads intones before directing you inside the fort.  Without hesitation you rush in.<br><br>", false);

outputText("You barely spare a thought at the interior in your hurry to get to the master.  As you burst into the room, the master's eyes light up at the sight of you.  \"<i>Ah!  It is my favorite fan.  Have you brought me a piece of that pink crystal I asked you for?</i>\"<br><br>", false);
...
outputText("You produce the pink crystal and hand it to the master.  As the crystal touches his hand, he gives a low growl of pleasure and holds the crystal up to his eyes.  \"<i>Ah yes, excellent!  Thank you so much for this tasty treat of power.</i>\"<br><br>", false);
//if (PC gave 2/3 Merae's crystal)
if (player.keyItemv1("Marae's Lethicite") == 2) {
    outputText("He looks at the crystal for a moment, before biting off half of it and giving the other half back to you.  \"<i>This is enough for the hellfire, I think.  Go ahead and keep the rest, my favorite fan!</i>\"<br><br>", false);
...
outputText("\"<i>Hum, hum, hum.</i>\"  He chuckles.  \"<i>The crystal of a goddess, this is going to bring changes, oh yes.</i>\"  The crystal flares up as he grips it more tightly.  \"<i>And you, oh you deserve so much more than just hellfire for this gift.  Come!</i>\"  He calls out.  In a moment, the biggest hellhound you've ever seen rushes into the room.<br><br>", false);

//if (PC has two dog dicks)
if (player.dogCocks() >= 2) {
    //the male bad end
    outputText("This hound is quite clearly a she, bearing a massive septuplet of breasts, and wide hips.  She is also quite clearly pregnant, and radiates an intense aura of power.  One of her two heads intones towards the crazed omnibus, \"<i>You called me master?</i>\"  \"<i>Yes I did my precious Cremera!  Have sex with this imitation at once!  " + player.mf("He", "She") + " is to become your third head!</i>\"<br><br>", false);

    outputText("Instantly Cremera turns around and presents to you her waiting sloppy twin cunts.  The hellhound master walks behind you and slaps you on the ass.  \"<i>Don't keep her waiting.</i>\"  He says as a shudder passes through you and your " + player.armorName + " falls to the ground around you.<br><br>", false);
    ...
    if (player.totalCocks() > 2 || player.cockThatFits(60) < 0) {
        outputText("He then reaches around your waist and takes a hold of your " + player.multiCockDescriptLight() + ".  \"<i>Before we get started, let's make sure you're just right for Cremera.</i>\"  He then reforms your body to have twin doggy pricks of appropriately sized.  \"<i>Now.</i>\"<br><br>", false);
    ...
    if (player.vaginalCapacity() < 45) {
        ...
        outputText("our two cunts quickly stretch larger to accommodate the intruders.<br><br>", false);
    }
    outputText("You let out a moan of pleasure from the pleasure of being filled.  The hound doesn't hesitate at all, and just thrusts into you with abandon.  You can't believe the feral feeling of it all, and that your twin womanhoods manage to so easily take the members and flow around them.<br><br>", false);
...
HellhoundScene.hellhoundPropahRape = function() {
    ...
    if (monster.HP < 1) outputText("spring to life, extending rapidly from the sheath. Tentatively you give one of them a gentle lick, being rewarded with a drop of pre-cum.<br><br>", false);
    else outputText("still manage to leak plenty of hot, steamy pre-cum all over his belly. Tentatively you give one of them a gentle lick, being rewarded with a dollop of the stuff.<br><br>", false);
//--- IF CORRUPTION < 20 ---
    if (player.cor < 20) {
