...
function GooGirl() {
    ...
    //Victory/defeat
    this.victory = GooGirlScene.gooGirlWin;
    this.defeat = GooGirlScene.gooGirlLoss;
}
...
GooGirl.prototype.outputDefaultTeaseReaction = function(lustDelta) {
    if (monster.lust <= 99) {
    if (lustDelta <= 0) outputText("<br><br>The goo-girl looks confused by your actions, as if she's trying to understand what you're doing.", false);
    else if (lustDelta < 13) outputText("<br><br>The curious goo has begun stroking herself openly, trying to understand the meaning of your actions by imitating you.", false);
    else outputText("<br>The girl begins to understand your intent. She opens and closes her mouth, as if panting, while she works slimy fingers between her thighs and across her jiggling nipples.", false);
}
else outputText("<br>It appears the goo-girl has gotten lost in her mimicry, squeezing her breasts and jilling her shiny " + monster.skinTone + " clit, her desire to investigate you forgotten.", false);
};
    ...
    if (player.gender == 0 || player.lust < 33 || gameFlags[SFW_MODE] > 0) {
        ...
        if (player.hasCock()) {
            if (player.cocks[player.smallestCockIndex()].cockLength < 24) {
                addButton(0, "DickSex", GooGirlScene.gooMaleRape, 2);
            }
            if (player.longestCockLength() >= 24) {
                addButton(1, "BigDickSex", GooGirlScene.gooMaleRape, 1);
            }
            if (player.hasVagina()) {
                addButton(2, "Herm Sex", GooGirlScene.victoryHermSex);
            }
        }
        else if (player.hasVagina()) {
            addButton(0, "Fem.Fuck", GooGirlScene.normalFemaleRapesAGooGirl);
        ...
        //if (valeria != null) {
        //    outputText("<br><br>Valeria's armored form seems to ebb towards the puddled goo-woman before you, almost eager to close the distance with her despite her pledge to protect you. ");
        ...
        if ((gameFlags[GOO_TFED_MEAN] == 0 && gameFlags[GOO_TFED_NICE] == 0) && gameFlags[TIMES_FUCKED_NORMAL_GOOS] >= 2) {
            if (player.cor < 50 && (player.hasItem(Items.Consumables.SuccubiMilk) || player.hasItem(Items.Consumables.SuccubiMilkPurified)) && (player.hasItem(Items.Consumables.BlackEgg) || player.hasItem(Items.Consumables.LBlackEgg))) {
                addButton(8, "Make Slave", GooGirlScene.pureGooRecruitmentStart);
                return;
            }
            else if (gameFlags[PC_KNOWS_ABOUT_BLACK_EGGS] > 0) {
                //Recruitment:
                //Notice After Victory:
                if (gameFlags[TIMES_THOUGHT_ABOUT_GOO_RECRUITMENT] == 0) {
                    outputText("<br><br>With this quivering lump of goo before you, a devilish idea comes to mind.  What would it take to make one of these aqueous sluts your willing slave?  ...Something to make them a little solid - perhaps one of those black eggs to make its skin a little more solid and restrainable?  Maybe a succubi milk to help make it a little more human-like, and the increased libido certainly couldn't hurt.");
                    gameFlags[TIMES_THOUGHT_ABOUT_GOO_RECRUITMENT]++;
                }
                //(Repeat)
                else {
                    outputText("<br><br>As you survey your victory, you remember the idea you had before - maybe if you drugged one of these things with a black egg and some succubi milk, you could make it your pet?");
                }
                if ((player.hasItem(Items.Consumables.SuccubiMilk) || player.hasItem(Items.Consumables.SuccubiMilkPurified)) && (player.hasItem(Items.Consumables.BlackEgg) || player.hasItem(Items.Consumables.LBlackEgg))) {
                    ...
                    addButton(8, "Make Slave", GooGirlScene.meanGooGirlRecruitment);
                }
            }
        }
*/
...
//Victory – Herm
GooGirlScene.victoryHermSex = function() {
    var x = player.biggestCockIndex();
    gameFlags[TIMES_FUCKED_NORMAL_GOOS]++;
    clearOutput();
    outputText("The goo-girl relents, her body sloshing into the puddle at her feet. Apparently convinced you want to explore her body, she composes her face into a willing smile and raises her arms at her sides, baring her " + monster.gooColor8() + " torso for inspection. The slime's chest juts out in perfect hemispheres, untouched by gravity and topped with puffy, " + monster.gooColor4() + " nipples that steadily drip a gooey imitation of milk. Her waist is whip-thin but broadens at her hips to accommodate her smooth, shiny rump, which she wiggles happily. Her hourglass figure would be life-threatening on any solid organism, but on the goo-girl, it merely appears she's been molded by someone with a dim grasp of anatomy.  Apparently, if slimes can't touch an organ, they don't consider it terribly important.<br><br>", false);

    outputText("Reaching out, you take hold of her breasts, the gelatinous orbs jiggling at your touch. Her slick membrane is strangely warm, curtains of heat playing across her skin in an attempt to speak a language you don't understand. You communicate in your own way by giving the girl a slap across her tits, the fluid mammaries rippling from the impact. Her mouth curls into a little 'o' of shocked pleasure and she nods in encouragement. Giving her another open-palmed smack, you notice that her breasts begin to enlarge, providing more surface area for you to strike. The slime seems to be getting off on the spanking, so you slap her again and again, " + monster.gooColor() + " jelly quivering in " + monster.gooColor5() + " tidal waves, her bloating chest swallowing her belly and waist as it inflates under your stimulation. Her body heaves in imitation of orgasmic panting and her arms sink into her oceanic bosom, squeezing the shimmering pillows together for you.<br><br>", false);

    outputText("Quickly stripping, you let your " + player.multiCockDescriptLight() + " slap against her turbulent membrane, the warm muck gurgling wetly at your turgid stiffness. Sinking your fingers into the girl's yielding breasts, you push forward, pulsing flesh sliding into the " + monster.gooColor2() + " depths of her expanded cleavage. The goo-girl's chest sucks eagerly at you, pliant slime drooling warm " + monster.gooColor5() + " beads of unnecessary lubrication as you pick up your pace. Staring at your " + player.multiCockDescriptLight() + " moving in and out of the girl's chest through her transparent skin, you are surprised to see her body shifting further under your pounding. A vertical slit opens between her breasts and her membrane quickly folds itself in delicate petals around the edges. Your next thrust pushes into the new cavity and the organic texture confirms your suspicion: she's created a vagina between her breasts for you to fuck!<br><br>", false);
...
// CORE DROP
GooGirlScene.coreDropChance = function() {
    if (rand(4) == 0 && gameFlags[SLIME_CRAVING] > 0 && player.findPerk(PerkLib.SlimeCore) < 0 && player.isGoo() && player.gooScore() >= 4) {
        outputText("<br><br>As the goo-girl slithers away, into the lake's placid waves, you notice she seems to have left behind a small blob. Upon investigation, it appears to be a tiny, ruby heart, encased in a slimy " + monster.gooColor8() + " membrane. As you reach to pick it up, the jelly ball quivers and pulses with a warm, cheerful light. Your fingers close on it and the nucleus slides through your palm, into your body!<br><br>", false);

        outputText("There is a momentary pressure in your chest and a few memories that are not your own flicker before your eyes. The dizzying sight passes and the slime core settles within your body, imprinted with your personality and experiences. There is a comforting calmness from your new nucleus and you feel as though, with your new memories, you will be better able to manage your body's fluid requirements.<br><br>", false);
...
GooGirlScene.layBeeEggsInGoo = function() {
    clearOutput();
...
// NEED WORMS TO FINISH
GooGirlScene.gooGirlLoss = function() {
    //if (pcCameWorms) {
    //    outputText("<br><br>The goo-girl seems confused but doesn't mind.");
    //    cleanupAfterCombat();
    // } else {
...
GooGirlScene.hermLoseToGooGal = function() {
    ...
    outputText("  Your spurting ecstasy floods the girl's jelly body, coloring her " + monster.gooColor3() + " body white as she drinks deeply, coaxing a few seconds of gasping spasms into long, seed-spewing minutes of climaxing euphoria.", false);
...
//[Pregnancy continued]
    if (player.pregnancyIncubation <= 280 && player.pregnancyIncubation >= 1) {
        ...
        else outputText("  You bite your lip and let go, torrents of rich, pearl fluid rushing through her arms and inflating the surprised girl before your eyes. The milk gushing into her fills the goo from base to breast, giving the once-" + monster.gooColor5() + " girl an opalescent shine, her over-filled body bloated as if she were carrying triplets.", false);
...
//[If the player isn't pregnant]
        ...
        outputText("  Your body is wildly distorted by the intruder, some aspect of her slimy coating anesthetizing your anticipated pain with tooth-grinding pleasure. You stroke your bloated form, thoughts turning to sweet daydreams of pregnancy, your body full of a pulsing, liquid life, dependent upon you and squirming with eager desire.<br><br>", false);
