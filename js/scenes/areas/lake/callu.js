...
CalluScene.ottahGirl = function() {
    ...
    if (gameFlags[MET_OTTERGIRL] == 1) {
        ...
        outputText("<br><br>\"<i>Name's Callu. Don't worry darlin', I don't plan on stickin' nothin' where it don't belong,</i>\" her soft voice chimes, \"<i>Unlike damn near everything else around here.</i>\"");
        outputText("<br><br>Well, how about it?");
    ...
    //Repeats
        ...
        outputText("<br><br>\"<i>Well ain't you a sight for sore eyes.</i>\" You sit down next to her and relax, just sitting and watching the makeshift bobber tip and sway in the water. \"<i>You up for a fish'n'fuck then?</i>\" she asks suddenly, brushing a strand of her sun bleached blonde hair out of her face.");
        outputText("<br><br>Well, are you?");
        //[Facesitting] [Fuck Her] [Fish] [Skedaddle]
    }
    menu();
    if (player.lust < 33) outputText("<br><br>You aren't aroused enough to fuck her.");
    else {
        //(If cocksize above 48")
        if (player.hasCock()) {
            if (player.shortestCockLength() > 48) outputText("<br><br>Unfortunately, you don't think she can quite handle your cock.");
            else addButton(0, "Fuck Her", CalluScene.ottergirlLikesDongs);
        }
        if (player.hasVagina() || !player.hasCock()) addButton(1, "Facesitting", CalluScene.ottersForGals);
    }
    if (gameFlags[MET_OTTERGIRL] > 1) addButton(2, "Get Fish", CalluScene.getSomeFishYaFatty);
    addButton(4, "Leave", CalluScene.avoidZeOtterPussy);
}
...
CalluScene.ottergirlLikesDongs = function() {
    ...
    outputText("<br><br>Strange, but you oblige, stripping off your [armor] and gear and tossing them aside. Callu instructs you to lay down on the beach next to her fishing pole, which you likewise oblige. The otter-girl straddles your stomach, facing away from you, though her thick, heavy tail is thankfully kept away from your face.");
    var x;
    var y = -1;
    var temp = 0;
    ...
    //(Under 6")
    if (player.cocks[x].cockLength < 6) outputText("<br><br>\"<i>Well butter my buns and call me a biscuit, ain't this a cute little thing,</i>\" she remarks, inspecting your tiny cock. \"<i>I ain't never seen one this small. I just wanna wrap it up in a little bow and cuddle with it. You sure it ain't a clit, darlin'?</i>\"");
    //(6"-10")
    else if (player.cocks[x].cockLength < 10) outputText("<br><br>\"<i>Just packin' the average model, eh? Nothin' wrong with that,</i>\" she remarks while inspecting your cock.");
    //(10"-24")
    else if (player.cocks[x].cockLength < 24) outputText("<br><br>\"<i>Oh my, now that's a manly piece of meat right there,</i>\" she remarks, inspecting your oversized cock. \"<i>I could enjoy that bad boy all day.</i>\"");
    //(24"-48")
    else outputText("<br><br>\"<i>Whoa nellie,</i>\" she says, her eyes going wide as they feast upon your giant cock. \"<i>That. That right there, darlin', is one grade-A trouser snake. I've seen centaurs that'd look like geldings next to you.</i>\"");
    ...
    outputText("<br><br>You feel slender but powerful fingers wrap around your cock, followed shortly after by a pair of lips. They encircle your " + player.cockHead(x) + " and suck, creating a delightful tingling sensation that travels down your cock and into your core.");

    outputText("<br><br>\"<i>Hey darlin', better get to lickin', we want this ");
    //{(lil dicks)
    ...
    outputText("<br><br>Likewise, her tongue and lips dance over your " + player.cockDescript(x) + " like a trio of dancers. They spin, twist, hop and tease, ensuring that no inch is left untouched.");
    ...
    outputText("<br><br>With your entire ");
    ...
    outputText("<br><br>To your surprise, Callu simply picks up where she left off");
...
CalluScene.ottersForGals = function() {
    ...
    outputText("<br><br>Callu leans down, laying her body across yours so that her warm, sweet-smelling cunt is positioned just in front of your face. Meanwhile, you feel delicate, powerful fingers probing at your [vagOrAss]. A long wet tongue licks over your ");
    ...
    outputText("<br><br>Likewise, her tongue and lips dance across your flesh like a trio of dancers. They twirl, spin, hop and tease. Not one inch is left untouched. From your ");
    ...
    outputText("<br><br>Grabbing hold of your [nipples], you start playing with them while Callu does her thing. Your fingers deftly tweak and tease them, knowing all the right techniques to really get you going. ");
    ...
    outputText("<br><br>The pleasure within you finally bursts outwards, shooting through every nerve, inflaming every fiber of your being. ");
...
CalluScene.getSomeFishYaFatty = function() {
    ...
    outputText("<br><br>She retrieves a delicious-smelling slab of roasted fish, properly salted and wrapped in a large green leaf. \"<i>Here ya're, fresh as it comes 'less you want it still walkin' and talkin'.</i>\"");
    outputText("<br><br>You thank Callu for the food and take your leave. ");
    Inventory.takeItem(Items.Consumables.FishFillet, Camp.returnToCampUseOneHour);
}