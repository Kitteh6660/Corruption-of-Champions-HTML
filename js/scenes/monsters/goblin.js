...
function Goblin() {
	...
	this.defeat = GoblinScene.defeatAgainstGoblin;
}
...
Goblin.goblinDrugAttack = function() {
	...
	if (color != "blue") {
		if (monster.refName == "Tamani's daughters") outputText("Tamani uncorks a glass bottle full of " + color + " fluid and swings her arm, flinging a wave of fluid at you.");
		else outputText(capitalize(monster.a) + monster.refName + " uncorks a glass bottle full of " + color + " fluid and swings her arm, flinging a wave of fluid at you.");
}
	//Drink blue pots
		...
		outputText("Tamani pulls out a blue vial and uncaps it, then douses the mob with the contents.");
			if (this.HPRatio() < 1) {
				outputText("  Though less effective than ingesting it, the potion looks to have helped the goblins recover from their wounds!<br>");
				this.changeHP(80 * multiplier);
			}
			else outputText("  There doesn't seem to be any effect.<br>");
			outputText("<br>");
			return;
		}
	else {
	...
	if ((player.findPerk(PerkLib.Evade) >= 0 && rand(10) <= 3) || (rand(100) < player.spe/5)) {
		outputText("<br>You narrowly avoid the gush of alchemic fluids!<br>");
	}
	else {
	//Get hit!
		if (color == "red") {
		//Temporary heat
			outputText("<br>The red fluids hit you and instantly soak into your skin, disappearing.  Your skin flushes and you feel warm.  Oh no...<br>");
		...
		//Green poison
			outputText("<br>The greenish fluids splash over you, making you feel slimy and gross.  Nausea plagues you immediately - you have been poisoned!<br>");
			if (player.findStatusEffect(StatusEffects.Poison) < 0) player.createStatusEffect(StatusEffects.Poison, 0, multiplier, 0, 0);
		}
		else if (color == "white") {
		//sticky flee prevention
			outputText("<br>You try to avoid it, but it splatters the ground around you with very sticky white fluid, making it difficult to run.  You'll have a hard time escaping now!<br>");
			if (player.findStatusEffect(StatusEffects.NoFlee) < 0) player.createStatusEffect(StatusEffects.NoFlee, 0, 0, 0, 0);
		}
		else if (color == "black") {
		//Increase fatigue
			outputText("<br>The black fluid splashes all over you and wicks into your skin near-instantly.  It makes you feel tired and drowsy.<br>");
...
GoblinScene.victoryAgainstGoblin = function() {
	...
	if (player.lust < 30 && player.findStatusEffect(StatusEffects.Feeder) < 0) { // Check this Status Effect check
		cleanupAfterCombat();
		return;
	}
	...
	if (player.lust >= 33 && player.gender > 0) {
		outputText("<br><br><b>What do you do to her, and if anything, which of your body parts do you use?</b>");
		if (player.hasCock()) {
			if (player.cockThatFits(monster.vaginalCapacity()) >= 0) {
				addButton(0, "Dick Fuck", GoblinScene.gatsGoblinBoners, null, null, null, "TO BE ADDED");
				addButton(4, "Jog Fuck", GoblinScene.gobboGetsRapedMaleFits, null, null, null, "TO BE ADDED");
			}
			if (player.cockArea(player.biggestCockIndex()) > monster.vaginalCapacity()) {
				addButton(1, "DickTooBig", GoblinScene.manRapesGoblinTooBig, null, null, null, "TO BE ADDED");
			}
			if (player.cockArea(player.biggestCockIndex()) > monster.vaginalCapacity() && player.cor > 80) { // && kGAMECLASS.monk > 2) FIGURE OUT WHAT THIS IS?
				addButton(2, "CorruptBig", GoblinScene.rapeAGoblinCorruptTooBig, null, null, null, "TO BE ADDED");
			}
			if (player.cockThatFits(monster.analCapacity()) >= 0 && (player.cor > 70 - player.corruptionTolerance() || gameFlags[MEANINGLESS_CORRUPTION] >= 1)) {
				addButton(3, "Dick In Ass", GoblinScene.goblinButtSex, null, null, null, "TO BE ADDED");
			}
			...
			if (player.tailType == TAIL_TYPE_SPIDER_ADBOMEN && player.cockThatFits(monster.vaginalCapacity()) >= 0) {
				...
				addButton(6, "Use Condom", GoblinScene.goblinCondomed, 1, null, null, "TO BE ADDED"); // Pass argument to do normal condom
			}
			
		}
		if (player.hasVagina()) {
			addButton(7, "Pussies", GoblinScene.goblinFemaleRape, null, null, null, "TO BE ADDED");
		}
		if (player.canOvipositSpider())	{
			addButton(8, "Lay Eggs", GoblinScene.goblinDriderEggs, null, null, null, "TO BE ADDED");
		}
	}

	// Not horny enough, but can feed or oviposit
	if (player.canOvipositSpider() || player.findStatusEffect(StatusEffects.Feeder) >= 0) { // CHECK STATUS EFFECT
		outputText("<br><br><b>You aren't horny enough to rape her, but ");
		if (player.findStatusEffect(StatusEffects.Feeder) >= 0) outputText("your nipples ache with the desire to feed her your milk.  Do you feed her milk or leave?</b>");
...
GoblinScene.goblinFemaleRape = function() {
	//spriteSelect(24);
	clearOutput();
	// Are you a taur?
	if (player.isTaur()) {
	...
	// Are you a naga?
		...
		//[Non-corrupt characters:
			...
			outputText("Opening your fingers, you open her mouth and prevent it from closing. The confused goblin tries to see what you're doing beside her but can't. When you bring up a handful of shiny flasks from her pouches though, her body tenses again and she whimpers at you.<br><br>");

			outputText("Your pity for the creature doesn't extend quite far enough to prevent you from punishing her though, and you pick out some of the less potent looking concoctions from the bunch. While stroking her throat gently to make sure she swallows you pour in vial after vial.<br><br>");
		}
		outputText("The effects don't take long to materialize, and soon the slut is a purplish hue and desperately trying to grind against your coils. She pants and moans in frustration as her dripping cunt can't get enough pleasure from your smooth underbelly, while you wait, enjoying the sensations.<br><br>");

		outputText("You slide your tail up to her cunt and tweak her clit with the tip. She immediately releases a gush of fluids, thoroughly coating your already slick and sticky coils in more of her cum. You waggle your finger in front of her face to tell her off; you're not done with her yet.<br><br>");
		...
		outputText("You pull it out and examine it. Not much more than a centimeter long, it's filled with a white fluid. There's a label, but the lettering is far too small to read. You think you can make out the word \"<i>wake</i>\", but aren't entirely sure.<br><br>");
		...
		outputText("That's proven very false in a moment though, as her eyes fly open and she yells out \"<i>WHOOOOOO!</i>\". You're startled at the sudden change in demeanor, but decide to roll with it, shoving your " + player.vaginaDescript(0) + " into the evidently very energetic little creature.<br><br>");

		outputText("You hear a muffled yell of \"<i>YOU GOT IT BOSS!</i>\" as she starts licking and gently chewing at you. The feeling is wonderful and you can't help but wonder what was in the vial, but the thought is wiped from your mind as you cum, spraying all over her.<br><br>");
		...
		outputText(".  Your arousal can wait.  This slut needs to be punished.<br><br>");
		...
		outputText("\"<i>YOU GOT ME WET, YOU CUNT!!  BAD BITCH!</i>\" you scold, slapping her even harder.<br><br>");
		outputText("The goblin squeals, though in delight or pain you can't be sure.  You start spanking her harder and harder, turning her ass from green to red with the repeated abuse.  Her entire body begins convulsing and squirting, splattering your arm with her honey.  She got off on it!  Well, after that kind of show, she's going to get you off too - or else!<br><br>");
...
// Dick too big
GoblinScene.manRapesGoblinTooBig = function() {
	clearOutput();
	...
	outputText(" absurd size to your victim.  The goblin-girl's eyes grow to the size of dinner plates in shock as she takes in the view.   Knowing you'll try regardless of the size-mismatch, she spreads her legs and settles herself more comfortably on the ground.<br><br>");
	outputText("You ");
	...
	outputText(", but after a moment or two of progress the tiny passage will stretch no more, and you're sure you can't force any more in without hurting her.<br><br>");
...
GoblinScene.rapeAGoblinCorruptTooBig = function() {
	var x = player.biggestCockIndex();
	clearOutput();
	//spriteSelect(24);
	...
	outputText("You reposition your fucktoy so that now she's facing away from you and again bring your " + player.cockDescript(x) + " into contact with her " + monster.vaginaDescript() + ". Again she struggles, but after her orgasm she doesn't have the strength to put up a decent fight; all her resistance does is arouse you further, now that she's fighting the inevitable. You begin to press her down onto yourself.<br><br>");

	outputText("\"<i>Too much...</i>\" she says, weakly. The tip of the head pops in.<br><br>");
	outputText("\"<i>You'll kill me...</i>\" she pleads. The rest of the head follows.<br><br>");
	outputText("\"<i>Stop...</i>\" she begs. The shaft starts to sink in.<br><br>");
	outputText("\"<i>Don't...</i>\" More enters her small body.<br><br>");
	outputText("\"<i>Please...</i>\" She fills to capacity.<br><br>");
	outputText("\"<i>Please...</i>\" And beyond.<br><br>");
	outputText("\"<i>... Fuck me.</i>\"<br><br>");
	...
	outputText(" the sweat of the both of you, and the only sound she makes is an occasional gurgle of ecstasy every few orgasms.<br><br>");
	...
	outputText("er spasming " + monster.vaginaDescript() + " is clamping down so hard on your " + player.cockDescript(x) + " that it can't release.<br><br>");
	...
	outputText("For what seems like hours you almost literally screw the brains out of her little green head, working yourself back up to the brink. You consider pulling out this time, but decide against it. At least two loads worth at once; it'll work, and the bitch has it coming.<br><br>");
	outputText("\"<i>Let's. Try. This. AGAIN!</i>\" you shout, pulling the two of you back into a sitting position and arching both of your backs.<br><br>");
	...
	outputText("The same tightness that prevented your cumming the first time now ensures that none of the copious amount of seed you blast into her escapes until you pull out. It's a good thing your corruption was so effective, as she is beginning to look pregnant enough to hold a beach ball.<br><br>");
	outputText("Finally you blow your last wad into this latest piece of ass, shoving her off of your " + player.cockDescript(x) + " and letting her fall to the ground. The impact sends torrents of cum sluicing out of her.<br><br>");
	outputText("You stand and prepare to leave, looking down at the goblin slut you just finished with.<br><br>");
	...
	outputText("<br><br>She absolutely will.");
...
GoblinScene.goblinButtSex = function() {
	//spriteSelect(24);
	var x = player.cockThatFits(monster.analCapacity());
	if (x < 0) x = 0;
	clearOutput();
	...
	outputText("\"<i>Fuck me, stud!</i>\" she begs. Though defeated, she has to gall to make demands. \"<i>Pump me full of your baby batter!</i>\" Her defeat doesn't seem to do much to silence her tongue.<br><br>");
	outputText("You're insulted. Who emerged victorious from the prior battle? You could have left her there to wallow in a pool of her own juices if you weren't so damn horny yourself. So what do you do? After some silent pondering, you get a deliciously wicked idea.<br><br>");
	outputText("After quickly removing your " + player.armorName + ", you tear off what little clothing the green-skinned woman is wearing.  With ease, you lift her off the ground and position her over your " + player.cockDescript(x) + ". The little goblin whore is so enthralled with you that her body quivers from excitement.<br><br>");
	outputText("With a sneer, you take the crown of your " + player.cockDescript(x) + " and press it not against her drenched pussy lips, but her asshole!<br><br>");
	outputText("\"<i>Not there!</i>\" she begs, her lips trembling and eyes watering. \"<i>I need your spunk to make my own tribe!</i>\" she explains.<br><br>");
	outputText("You say nothing to the trembling woman in your grasp. No words could describe the joy you feel from crushing her dreams. By the elders, this world has truly corrupted you, and you love it!<br><br>");
	outputText("Without any reservations, you slam the goblin whore onto your " + player.cockDescript(x) + ", virtually impaling her! The immediate tightness of her asshole nearly drives you over the edge! The more she squirms, the tighter her ass muscles squeeze you. You stand frozen in ecstasy for a moment, your tongue drooping out of your mouth and eyes rolling into the back of your head.<br><br>");
	...
	outputText(" smacking her plump rump with each thrust, as if to tease her.<br><br>");
	outputText("The tight confines of the goblin's asshole prove too much for you. Your body convulses wildly as you unload a massive load in her. Spent, you throw the little whore onto the ground; you have no further use for her at the moment.<br><br>");
...
GoblinScene.giveGoblinAMilkMustache = function() {
	...
	player.addStatusValue(StatusEffects.Feeder,1,1); // CHECK THIS
	player.changeStatusValue(StatusEffects.Feeder,2,0); // CHECK THIS
	cleanupAfterCombat();
};

// Spider condom or regular condom
GoblinScene.goblinCondomed = function(mode) {
	//spriteSelect(24);
	var x = player.cockThatFits(monster.vaginalCapacity());
	clearOutput();
	...
	outputText(" Time to give the goblin what she asked for, if not what she wanted.<br><br>");
	...
	outputText("Then, wresting her feet apart, you unceremoniously dump her to the ground.<br><br>");
...
GoblinScene.goblinDriderEggs = function() {
	clearOutput();
	...
	outputText("<br><br>You quickly undress and skitter over to the sniffling slut, reaching down to pull her up and turning her to face you as you do.  Looping one arm under her armpits, you prod at her soft stomach with your free hand, inquiring about the state of her children.");

	//[if (femininity > 50)
	if (player.femininity > 50) outputText("<br><br>\"<i>W-what?  You- I haven't got any, you stupid bitch!  This your idea of fun, jackass?  Kicking people when they're down?!</i>\"");
	//[if (femininity < 51) 
	else outputText("<br><br>\"<i>W-what?  You- I haven't got any, you stupid bastard!  This your idea of fun, jackass?  Kicking people when they're down?!</i>\"");
	...
	outputText("<br><br>Then the next egg rolls forward, and the next, and the next...");

	outputText("<br><br>You keep her pinned against your body as you fill her up, one orb at a time, each sphere bloating her stretched stomach a little further, until she's so full you can feel your eggs through the taut skin of her belly.  The goblin is nearly unconscious, insensibly gurgling as the pleasure of her instant pregnancy numbs her mind.  You pull her off with a loud wet plop, her twitching snatch leaking an unending stream of her own clear fluids as well as a sticky string of your green egg-mucus.");

	outputText("<br><br>Laying her down in the shade, you put your clothes back on, glad to be free of the extra weight and ready to continue your adventure.");
	player.dumpEggs();
	cleanupAfterCombat();
	player.orgasm();
};
...
GoblinScene.defeatAgainstGoblin = function() {
	...
	else if (player.findStatusEffect(StatusEffects.Infested) >= 0) {
		outputText("<br><br>The goblin's eyes go wide and she turns to leave, no longer interested in you.");
...
// Goblin rapes player.
GoblinScene.goblinRapesPlayer = function() {
	...
	if (player.findPerk(PerkLib.BimboBrains) >= 0 || player.findPerk(PerkLib.FutaFaculties) >= 0) {
		...
		if (player.hasVagina()) {
			...
			outputText("The goblin, almost drooling, says \"<i>Baby, we're both horny, but fingers and tongues just aren't enough for me. Lucky for you, I've got just the thing.</i>\"<br><br>");

			outputText("For a moment, your dumb, bimbo mind struggles to think of whatever she could, like, mean by that. Oh well, you giggle airheadedly and just let it happen.<br><br>");
			...
			outputText("For a moment, you blink dumbly, trying to think why for, like, one tiny second you wanted to say 'no', but then you forget about it, let the horniness flow, and naturally pop your mouth open into a perfect-lip lined 'O', shooting her a involuntarily slutty look.<br><br>");
	...
	outputText(" form.  Your breath is knocked from your body as she drops all her weight onto your middle, making her ass and well-rounded tits jiggle enticingly.   She reaches up to twist her nipple-studs, grinding the sopping wetness of her crotch up and down your belly.<br><br>");
	...
	outputText(", torturing you with pain and pleasure.<br><br>");
	...
	if (player.totalCocks() > 0 && (!player.hasVagina() || rand(2) == 0)) {
		...
		if (player.cockArea(0) > monster.vaginalCapacity()) {
			...
			outputText(".  The goblin looks back over her shoulder, narrows her eyes, and pulls back to say, \"<i>Don't even think about holding back stud.</i>\"<br><br>");
			...
			outputText(" as she whispers, \"<i>Go ahead, make my twat a swollen cum-dump.  I NEED you to FILL me with ALL of your cream.  Stuff me full of your fuck-juice and I promise I'll give you a dozen slutty daughters to fill with jizz every night.</i>\"<br><br>");
			...
			outputText("  Satisfied, your orgasm tapers off into tiny dribbles.  The goblin slips off you, looking a bit bowlegged, but utterly pleased.<br><br>");
		...
		//Dick fits...
			...
			outputText(", spreading around you ever-so-slowly as she sinks further and further down.  She licks her glossy lips and blows you a kiss as she violently pushes the rest of the way down, impaling herself on every inch of your length.<br><br>");
			...
			outputText("The sultry slut pulls out a flask from somewhere behind her and uncorks it, releasing a puff of red smoke that smells of cherries.<br><br>");
			...
			outputText("  A sensation of dizziness flows through you, along with relaxing waves of gentle warmth that make it easy to relax and let that cute green whore take your shaft.<br><br>");
	...
	// Female rape, or herm pussy rape.
		...
		outputText("The goblin says, \"<i>Baby we're both horny, but fingers and tongues just aren't enough for me. Lucky for you, I've got just the thing.</i>\"<br><br>");
		...
		outputText("  It 'plumps' up somehow - perhaps in reaction to your spit - forcing your jaw open and pinning your tongue to the bottom of your mouth.  A trickle of fluid escapes its tip, nearly gagging you before your throat reflexively drinks it down.  What did you just swallow?<br><br>");
		outputText("\"<i>Yummy yummy isn't it?  I made this myself.  It's made up of the best stuff – it reacts with fluids to puff up and fill ANY hole perfectly.  Even better, it has a reservoir stuffed with aphrodisiacs that'll slowly leak out.  Do you feel warm yet hun?</i>\" she asks.<br><br>");
