...
addToGameFlags(GIACOMO_MET, GIACOMO_WORMS_OFFERED);

GiacomoScene.giacomoEncounter = function() {
    displaySprite("giacomo");
    clearOutput();
    if (gameFlags[GIACOMO_MET] == 0) {
        ...
        outputText("\"<i>Do not look at me as if I am a mystic,</i>\" Giacomo heckles lightly. \"<i>Your crotch is squirming.</i>\"<br><br>");
        outputText("Looking down, you realize how right he is and attempt to cover yourself in embarrassment.<br><br>");
        outputText("\"<i>Fear not!</i>\" the purveyor jingles. \"<i>I have something that will cure you of those little bastards. Of course, there is also a chance that it will purge your system in general. This potion is not cheap. I will trade it for 175 gems.</i>\"<br><br>");
        gameFlags[GIACOMO_WORMS_OFFERED] = 1;
        if (player.gems < 175) { //Broke as a joke
            ...
            //Remove/No
            doYesNo(GiacomoScene.wormRemoval, GiacomoScene.giacomoEncounter);
            return;
        }
    }
    ...
    menu();
    addButton(0, "Potions", GiacomoScene.potionMenu);
    addButton(1, "Books", GiacomoScene.bookMenu);
    addButton(2, "Erotica", GiacomoScene.eroticaMenu);
    addButton(3, "Worm Cure", GiacomoScene.wormRemovalOffer);
    addButton(4, "Leave", Camp.returnToCampUseOneHour);
}

GiacomoScene.firstEncounter = function() {
    ...
    outputText("\"<i>Greetings, traveler! My name is Giacomo. I am, as you can see, a humble purveyor of items, curios and other accoutrements. While I am not in a position to show you my full wares as my shop is packed on this push-cart, I do offer some small trinkets for travelers I meet.</i>\"<br><br>");
    outputText("The merchant looks at you sharply and cracks a wide, toothy smile you find... unnerving. The merchant twists his way around to access a sack he has around his back. After a moment, he swings the sack from his back to have better access to its contents. Inquisitively, the merchant turns back to you.<br>");
    outputText("\"<i>So stranger, be you interested in some drafts to aid you in your travels, some quick pamphlets to warn you of dangers on journeys or...</i>\"<br><br>");
    outputText("Giacomo pauses and turns his head in both directions in a mocking gesture of paranoid observation. His little bit of theatrics does make you wonder what he is about to offer.<br>");
    outputText("\"<i>...maybe you would be interested in some items that enhance the pleasures of the flesh? Hmmm?</i>\"<br><br>");
    outputText("Giacomo's grin is nothing short of creepy as he offers his wares to you. What are you interested in?");
    gameFlags[GIACOMO_MET] = 1;
}

//------------
// MENUS
//------------
GiacomoScene.potionMenu = function() {
    clearOutput();
    outputText("Which potion or tincture will you examine?");
    menu();
    addButton(0, "Vitality T.", GiacomoScene.pitchVitailtyTincture);
    addButton(1, "Scholars T.", GiacomoScene.pitchScholarsTea);
    //addButton(2, "Cerulean P.", GiacomoScene.pitchCeruleanPotion);
    addButton(4, "Back", GiacomoScene.giacomoEncounter);
}

GiacomoScene.bookMenu = function() {
    clearOutput();
    outputText("Which book are you interested in perusing?");
    menu();
    addButton(0, "Dangerous Plants", GiacomoScene.pitchDangerousPlantsBook);
    addButton(1, "Traveler's Guide", GiacomoScene.pitchTravellersGuide);
    addButton(2, "Hentai Comic", GiacomoScene.pitchHentaiComic);
    addButton(3, "Yoga Guide", GiacomoScene.pitchYogaGuide);
    addButton(4, "Back", GiacomoScene.giacomoEncounter);
}

GiacomoScene.eroticaMenu = function() {
    clearOutput();
    outputText("Giacomo's grin is nothing short of creepy as he offers his wares to you. What are you interested in?");
    menu();
    addButton(0, "Dildo", GiacomoScene.pitchDildo);
    if (player.hasVagina()) addButton(1, "Stim-Belt", GiacomoScene.pitchSelfStimulationBelt);
    if (player.hasVagina()) addButton(2, "AN Stim-Belt", GiacomoScene.pitchAllNaturalSelfStimulationBelt);
    if (player.hasCock()) addButton(3, "Onahole", GiacomoScene.pitchOnahole);
    if (player.hasCock()) addButton(4, "D Onahole", GiacomoScene.pitchDeluxeOnahole);
    if (player.hasCock() && player.hasVagina()) addButton(5, "Dual Belt", GiacomoScene.pitchDualStimulationBelt);
    if (player.hasCock() && player.hasVagina()) addButton(6, "AN Onahole", GiacomoScene.pitchAllNaturalOnahole);
    addButton(7, "Condom", GiacomoScene.pitchCondom);
    addButton(14, "Back", GiacomoScene.giacomoEncounter);
}

//------------
// CONSUMABLES
//------------
GiacomoScene.pitchVitailtyTincture = function() {
    clearOutput();
    ...
    doYesNo(GiacomoScene.buyVitailtyTincture, GiacomoScene.potionMenu);
}
GiacomoScene.buyVitailtyTincture = function() {
    if (player.gems < 15) {
        clearOutput();
        outputText("<br><br>Giacomo sighs, indicating you need " + num2Text(15 - player.gems) + " more gems to purchase this item.");
        doNext(GiacomoScene.potionMenu);
    }
    else {
        player.changeGems(-15);
        Inventory.takeItem(Items.Consumables.VitalityTincture, GiacomoScene.potionMenu);
    }
}

GiacomoScene.pitchScholarsTea = function() {
    clearOutput();
    ...
    doYesNo(GiacomoScene.buyScholarsTea, GiacomoScene.potionMenu);
}
GiacomoScene.buyScholarsTea = function() {
    if (player.gems < 15) {
        clearOutput();
        outputText("<br><br>Giacomo sighs, indicating you need " + num2Text(15 - player.gems) + " more gems to purchase this item. ");
        doNext(GiacomoScene.potionMenu);
        ...
        player.changeGems(-15);
        Inventory.takeItem(Items.Consumables.ScholarsTea, GiacomoScene.potionMenu);
    }
}

GiacomoScene.pitchCeruleanPotion = function() {
    clearOutput();
    ...
    doYesNo(GiacomoScene.buyCeruleanPotion, GiacomoScene.potionMenu);
}
GiacomoScene.buyCeruleanPotion = function() {
    if (player.gems < 75) {
        clearOutput();
        outputText("<br><br>Giacomo sighs, indicating you need " + num2Text(75 - player.gems) + " more gems to purchase this item.");
        doNext(GiacomoScene.potionMenu);
    }
    else {
        player.changeGems(-75);
        Inventory.takeItem(Items.Consumables.CeruleanPotion, GiacomoScene.potionMenu);
    }
}

GiacomoScene.pitchCondom = function() {
    outputText("Giacomo holds up the packet and says, \"<i>Ah, yes! This is a condom. Just slip it on any cocks and have it penetrate any holes. It's guaranteed to prevent the spread of STDs and it will help to prevent pregnancy. I must warn you that it does not completely prevent pregnancy. Rarely, it will fail. However, it will work most of the time. So, <b>two gems</b>. What do you say?</i>\"", true);
    doYesNo(GiacomoScene.buyCondom, GiacomoScene.eroticaMenu);
}
GiacomoScene.buyCondom = function() {
    if (player.gems < 2) {
        outputText("<br><br>Giacomo sighs, indicating you need " + num2Text(5 - player.gems) + " more gems to purchase this item.", true);
        doNext(GiacomoScene.eroticaMenu);
    }
    else
    {
        player.changeGems(-2);
        Inventory.takeItem(Items.Usables.Condom, GiacomoScene.eroticaMenu);
    }
}
...
// BOOKS
//------------
GiacomoScene.pitchDangerousPlantsBook = function() {
    clearOutput();
    if (player.hasKeyItem(KeyItems.BookDangerousPlants) >= 0) {
        outputText("<b>You already own the book 'Dangerous Plants'.</b>");
        doNext(GiacomoScene.bookMenu);
        return;
    }
...
GiacomoScene.pitchTravellersGuide = function() {
    ...
    doYesNo(GiacomoScene.buyTravellersGuide, GiacomoScene.bookMenu);
}
GiacomoScene.buyTravellersGuide = function() {
    clearOutput();
    if (player.gems < 1) {
        outputText("<br><br>Giacomo sighs, indicating you need 1 gem to purchase this item.");
    }
    else {
        ...
        player.changeGems(-1);
        player.createKeyItem(KeyItems.BookTravelGuide, 0, 0, 0, 0);
    }
    doNext(GiacomoScene.bookMenu);
}

GiacomoScene.pitchHentaiComic = function() {
    clearOutput();
    if (player.hasKeyItem(KeyItems.BookHentaiComic) >= 0)
    {
    ...
    doYesNo(GiacomoScene.buyHentaiComic, GiacomoScene.bookMenu);
}
GiacomoScene.buyHentaiComic = function() {
    clearOutput();
    if (player.gems < 10) {
        outputText("<br><br>Giacomo sighs, indicating you need " + num2Text(10 - player.gems) + " more gems to purchase this item.");
    }
        ...
        player.changeLust(20, true);
        player.changeGems(-10);
        player.createKeyItem(KeyItems.BookHentaiComic, 0, 0, 0, 0);
    }
    doNext(GiacomoScene.bookMenu);
}

GiacomoScene.pitchYogaGuide = function() {
    clearOutput();
    if (player.hasKeyItem(KeyItems.BookYogaGuide) >= 0) {
    ...
    doYesNo(GiacomoScene.buyYogaGuide, GiacomoScene.bookMenu);
}
GiacomoScene.buyYogaGuide = function() {
    clearOutput();
    if (player.gems < 100) {
        outputText("<br><br>Giacomo sighs, indicating you need " + num2Text(100 - player.gems) + " more gems to purchase this item.");
    }
    else {
        outputText("You exchange 100 gems for the tome.");
        player.changeGems(-100);
        player.createKeyItem(KeyItems.BookYogaGuide, 0, 0, 0, 0);
    }
    doNext(GiacomoScene.bookMenu);
}
...
// TOYS
//------------
GiacomoScene.pitchDildo = function() {
    clearOutput();
    if (player.hasKeyItem(KeyItems.ToyDildo) >= 0) {
    ...
    doYesNo(GiacomoScene.buyDildo, GiacomoScene.eroticaMenu);
}
GiacomoScene.buyDildo = function() {
    clearOutput();
    if (player.gems < 20) {
        outputText("<br><br>Giacomo sighs, indicating you need " + num2Text(20 - player.gems) + " more gems to purchase this item.");
    }
    else {
        outputText("After making the payment, Giacomo hands you the Dildo");
        player.changeGems(-20);
        player.createKeyItem(KeyItems.ToyDildo, 0, 0, 0, 0);
    }
    doNext(GiacomoScene.eroticaMenu);
}

GiacomoScene.pitchSelfStimulationBelt = function() {
    clearOutput();
    if (player.hasKeyItem(KeyItems.ToyStimBelt) >= 0) {
        outputText("<b>You already own a Self-Stimulation Belt!</b>");
        doNext(KeyItems.eroticaMenu);
        return;
    }
    ...
    doYesNo(KeyItems.buySelfStimulationBelt, KeyItems.eroticaMenu);
}
GiacomoScene.buySelfStimulationBelt = function() {
    clearOutput();
    if (player.gems < 30) {
        outputText("<br><br>Giacomo sighs, indicating you need " + num2Text(30 - player.gems) + " more gems to purchase this item.");
    }
    else {
        outputText("After making the payment, Giacomo hands you the Self-Stimulation Belt");
        player.changeGems(-30);
        player.createKeyItem(KeyItems.ToyStimBelt, 0, 0, 0, 0);
        doNext(GiacomoScene.eroticaMenu);
    }
    doNext(GiacomoScene.eroticaMenu);
}

GiacomoScene.pitchAllNaturalSelfStimulationBelt = function() {
    clearOutput();
    if (player.hasKeyItem(KeyItems.ToyANBelt) >= 0) {
    ...
    if (player.hasKeyItem(KeyItems.BookDangerousPlants) >= 0 && player.inte > 39) {
        outputText("<br><br>The nodule and the base of the stimulator look vaguely like some of the things you have seen in the Dangerous Plant book. You wonder if there is not something devious about this item. Giacomo is also sweating. It is too cool for that, this time of year.");
    }
    else {
        if (player.inte > 29) {
            outputText("<br><br>You notice a change in Giacomo's attitude. He REALLY wants to sell you this sex toy. Something this exotic should cost much more than what he is offering.");
        }
    }
    outputText("<br><br>Do you buy the All-Natural Self-Stimulation Belt?");
    doYesNo(GiacomoScene.buyAllNaturalSelfStimulationBelt, GiacomoScene.eroticaMenu);
}
GiacomoScene.buyAllNaturalSelfStimulationBelt = function() {
    clearOutput();
    if (player.gems < 40) {
        outputText("<br><br>Giacomo sighs, indicating you need " + num2Text(40 - player.gems) + " more gems to purchase this item.");
    }
    else {
        outputText("After making the payment, Giacomo hands you the All-Natural Self-Stimulation Belt");
        player.changeGems(-40);
        player.createKeyItem(KeyItems.ToyANBelt, 0, 0, 0, 0);
    }
    doNext(GiacomoScene.eroticaMenu);
}

GiacomoScene.pitchOnahole = function() {
    clearOutput();
    if (player.hasKeyItem(KeyItems.ToyOnahole) >= 0) {
    ...
    doYesNo(GiacomoScene.buyOnahole, GiacomoScene.eroticaMenu);
}
GiacomoScene.buyOnahole = function() {
    clearOutput();
    if (player.gems < 20) {
        outputText("<br><br>Giacomo sighs, indicating you need " + num2Text(20 - player.gems) + " more gems to purchase this item.");
    }
        ...
        player.changeGems(-20);
        player.createKeyItem(KeyItems.ToyOnahole, 0, 0, 0, 0);
    }
    doNext(GiacomoScene.eroticaMenu);
}

GiacomoScene.pitchDeluxeOnahole = function() {
    clearOutput();
    if (player.hasKeyItem(KeyItems.ToyDeluxeOnahole) >= 0) {
...
GiacomoScene.pitchAllNaturalOnahole = function() {
    ...
    doYesNo(GiacomoScene.buyAllNaturalOnahole, GiacomoScene.eroticaMenu);
}
GiacomoScene.buyAllNaturalOnahole = function() {
    clearOutput();
    if (player.gems < 150) {
        outputText("<br><br>Giacomo sighs, indicating you need " + num2Text(150 - player.gems) + " more gems to purchase this item.");
    }
    else {
        outputText("After making the payment, Giacomo hands you the All-Natural Onahole");
        player.changeGems(-150);
...
GiacomoScene.pitchDualStimulationBelt = function() {
    clearOutput();
    if (player.hasKeyItem(KeyItems.ToyDualBelt) >= 0) {
    ...
    doYesNo(GiacomoScene.buyDualStimulationBelt, GiacomoScene.eroticaMenu);
}
GiacomoScene.buyDualStimulationBelt = function() {
    clearOutput();
    if (player.gems < 50) {
        outputText("<br><br>Giacomo sighs, indicating you need " + num2Text(50 - player.gems) + " more gems to purchase this item.");
    }
        ...
        player.changeGems(-150);
        player.createKeyItem(KeyItems.ToyDualBelt, 0, 0, 0, 0);

    }
    doNext(GiacomoScene.eroticaMenu);
}

GiacomoScene.wormRemovalOffer = function() {
    clearOutput();
    outputText("<br><br>\"<i>Been playing with creatures best left alone, I see</i>\", he chuckles, \"<i>Infestations of any kind are annoying, yet your plight is quite challenging given the magnitude of corrupt creatures around here. It is not the first time I have seen one infested with THOSE worms.</i>\"<br><br>");
    outputText("You ask how he knows of your change and the merchant giggles heartily.<br><br>");
    outputText("\"<i>Do not look at me as if I am a mystic.</i>\", Giacomo heckles lightly. \"<i>Your crotch is squirming.</i>\"<br><br>");
    outputText("Looking down, you realize how right he is and attempt to cover yourself in embarrassment.<br><br>");
    outputText("\"<i>Fear not!</i>\", the purveyor jingles. \"<i>I have something that will cure you of those little bastards. Of course, there is also a chance that it will purge your system in general. This potion is not cheap. I will trade it for 175 gems.</i>\"<br><br>");
    ...
    if (player.gems < 175) {
        outputText("You realize you don't have enough gems for such a pricey potion, but perhaps there is something else in his inventory you can buy.");
        doNext(GiacomoScene.giacomoEncounter);
    }
    //Can afford
    else {
        outputText("Do you purchase his cure?");
...
GiacomoScene.wormRemoval = function() {
    clearOutput();
    ...
    outputText("Upon seeing the fat worm, Giacomo displays a freakish celerity by jumping off his cart, grabbing an empty container and collecting the fat worm. Regaining your senses, you look at him with inquisitive shock at what he just did.<br><br>");
    outputText("\"<i>You have to realize that I AM a merchant, after all.</i>\", he calmly replies. \"<i>Hell for you is heaven for someone else. This bugger will easily fetch 10,000 gems to some noble looking for a quick buzz. Don't WE know better!</i>\"<br><br>");
    outputText("The merchant puts away his prize and comes back to help you up. \"<i>Here.</i>\", he says as he shoves a couple of bottles into your hand. \"<i>This is on the house. You probably need it after the shock of getting those things out.</i>\"<br><br>");
    ...
    player.modStats("lib", -1, "cor", -8);
    player.changeGems(-175);
    Inventory.takeItem(Items.Consumables.VitalityTincture, Camp.returnToCampUseOneHour);
}

/*GiacomoScene.nightSuccubiFirstTime = function() {
    spriteSelect(8);
outputText("<br>As you sleep, your rest becomes increasingly disturbed. You feel a great weight on top of you and you find it difficult to breathe. Stirred to consciousness, your eyes are greeted by an enormous pair of blue tinged breasts. The nipples are quite long and thick and are surrounded by large, round areola. A deep, feminine voice breaks the silence. \"<i>I was wondering if you would wake up.</i>\" You turn your head to the voice to see the visage of a sharp-featured, attractive woman. The woman grins mischievously and speaks again. \"<i>I was hoping that idiot, Giacomo, did not dilute the 'potion' again.</i>\" Your campfire reflects off the woman's face and her beauty contains some sharply contrasting features. The pupils of her eyes are slit like a cat's. As she grins, she bares her teeth, which contain two pairs of long and short fangs. This woman is clearly NOT human! In shock, you attempt to get up, only prompting the woman to prove her inhuman nature by grabbing your shoulders and pinning you to the ground. You see that each finger on her hand also contains a fourth joint, further proving her status. Before you can speak a word, the woman begins mocking your fear and places her face in front of yours. Her face is almost certainly demonic in nature.<br><br>");
if (player.gender == 0) {
    outputText("She quickly moves down to your crotch... only to discover no organs down there.<br><br>");
    outputText("*record scratch*<br><br>");
    outputText("\"<i>Wait a fucking minute,</i>\" the Succubus says, \"<i>Where's your dick?!</i>\"<br><br>");
    outputText("As you state your genderless nature, the succubus hops off and from nowhere pulls out a large folder marked \"<i>Corruption of Champions-Script</i>\" and begins thumbing through the pages. After finding the page she is looking for, she reads it and looks off into the distance in disgust.<br><br>");
    outputText("\"<i>Hey Fenoxo and Dxasmodeus!!!!!!</i>\" the Succubus crows, \"<i>The goddamn script says that I should be milking someone's DICK!!! Man, futa, herm, I don't give a shit. YOUR OWN FUCKING SCRIPT SAYS I SHOULD BE MOUNTING AND MILKING A COCK!!!! THIS IS A SEX GAME!!!!!! THAT MEANS FUCKING! WHAT THE HELL AM I SUPPOSED TO FUCK???!!!</i>\"<br><br>");
    outputText("The Succubus looks at you with utter contempt, \"<i>THIS motherfucker doesn't have a DAMN thing! What am I supposed to do?! I can't exactly order a fucking Happy Meal!!!!!</i>\"<br><br>");
    outputText("Throwing the script down in an utter rage, the tantrum continues, \"<i>Goddammit! I can't believe this shit! HEY!!!!! INTERN!!!! Bring me my robe, aspirins and cancer sticks!!!!</i>\"<br><br>");
    outputText("The Succubus walks a few paces away where a plain-dressed woman with a clipboard hands the Succubus a pack of cigarettes and a small bottle of aspirin. She takes a fistful of the painkillers and immediately lights up a smoke. The Succubus takes a couple of drags off the cig and rubs her temples.<br><br>");
    outputText("\"<i>You two are killing me!</i>\" she groans in clear frustration, \"<i>I come to work for you perverts based off the promise of MORE perverts to feed from and you do THIS to me! I can't work like this!</i>\"<br><br>");
    outputText("The plain woman hands the Succubus a robe, which she crudely puts on as she storms off into the night.<br><br>");
    outputText("\"<i>I will discuss this horseshit with my agent,</i>\" the Succubus continues bitching, \"<i>THIS was NOT in my contract.</i>\"<br><br>");
    outputText("The Succubus stops, turns and points to you in derision. \"<i>And YOU! You no-cock, no-cunt having pissant! Take your ass back to the lab before they find out you escaped!!!!!</i>\"<br><br>");
...
GiacomoScene.ceruleanSuccubusEncounterPart4 = function() {
    spriteSelect(8);
if (player.gender == 1) {
    clearOutput();
...
GiacomoScene.nightSuccubiRepeat = function() {
    spriteSelect(8);
if (player.gender == 0) {
    ...
    //REPEAT
        ...
        outputText("<br>As you begin to relax, you hear footsteps behind you, expecting the unholy interloper and pray for a better... and more understanding... encounter.<br><br>");

        outputText("You turn around, hoping for an exciting encounter only to find a rather short, plain-faced woman with horned-rim glasses and a purple dress on. She appears to be holding a stack of papers in her hand.<br><br>");

        outputText("\"<i>Ahem.</i>\", the woman says meekly, \"<i>I hate to bother you, but I was sent by the CoC writers and staff to hand you this.</i>\"<br><br>");

        outputText("Scratching your head, you inquire what the document is. The woman smiles shyly and hands it to you.<br><br>");

        outputText("\"<i>This is the script and production notes for Corruption of Champions,</i>\" she says with a small bit of pride, \"<i>Apparently, you need to read the highlighted sections. They are important.</i>\"<br><br>");

        outputText("You take the script, scratching your head at the surreal nature of the moment. You thumb through the pages, finding virtually every aspect of your life and encounters written as if foreseen by great mystics. The accuracy is nothing short of horrifying. You find a highlighted section that appears to be what the woman is referring to. The note is terse and outright blunt.<br><br>");

        outputText("\"<i>GENDER NEUTRAL CHARACTERS ARE BUTT-MONKEYS. IF THE ENCOUNTER INVOLVES SEX, EXPECT SOMETHING FUCKED UP TO HAPPEN INSTEAD. ACTORS WHO PLAY NEUTER CHARACTERS SHOULD EXPECT TO PLAY ONLY FOR LULZ</i>.\"<br><br>");

        outputText("The shock is overwhelming. The script basically says that you will never catch a break. As this reality drapes about you, the script disappears and you hear a cacophony of mocking laughter in all directions. The woman is nowhere to be found.<br><br>");

        outputText("As the cacophony fades, you only hear one facetiously toned word,<br><br>");
...
if (player.gender == 1) {
    if (player.cor < 66) {
        ...
        outputText(" her contractions milking your " + player.cockDescript(0) + " as fiercely as a maid milks a cow! Another torrent of cum pushes its way out of your body and you let out a moan of pleasure and exhaustion.<br><br>");
        outputText("As you are passing out, you feel a deep kiss upon your lips from the succubus. \"You taste better each time we join. Call upon me soon, lest I take what I want from you anyway,\", says the lustful creature.<br><br>");
        outputText("Fatigue takes you and you collapse into a deep sleep. ");
    }
    else {
        outputText("<br>Knowing the succubus will come, you do not even bother trying to sleep. Instead, you prepare a little surprise for her. You briefly jerk off and start edging yourself, preparing a massive batch to unload inside her. Hopefully, she will be the one to get more than she bargained for.<br><br>", true);
        outputText("The succubus comes, as you predicted. Despite her obvious strength and size difference to you, you grab her and push her down to the ground and immediately push your angry cock into her hairy hole. The succubus, surprised and enthralled, laughs at your aggression.<br><br>");
        outputText("\"<i>I thought I was the hungry one.</i>\", she chuckles. \"I am all yours, little man. FEED ME!\"<br><br>");
        outputText("You begin bucking your ");
        if (player.isTaur()) outputText("flanks");
        else outputText("hips");
        outputText(" in the all-too-familiar rhythm, hammering away at the succubus' cunt. Impressed with your initiative, she chooses to remain submissive as you work up an impressive load of spunk. Trying with all of your might, you continue to hold off your orgasm, painfully, as you continue stimulating your inhuman lover.<br><br>");
        ...
        outputText(". You feel the head of your prick push past the dilated opening in her cervix, which immediately contracts around your head. Your penis is literally trapped and caught in her womb!<br><br>");
        ...
        outputText(", as if to force yourself to cum harder than you already are capable of, prompting an equally pleasurable reaction from her.<br><br>");
    ...
    if (flags[kFLAGS.CERULEAN_POTION_BAD_END_FUTA_COUNTER] > 10 && player.cor > 50) {
        outputText("<br>As the Succubus mounts you, an uncontrollable urge takes over your mind and body. Without any thought, you quickly thrust one of her nipples in your mouth and begin suckling wildly like a newborn child. The Succubus cries in shock and pleasure as you begin feeding from her and quickly begins her ritualistic milking of your dong. The warm milk passes into your mouth and down your throat, where it settles peacefully in your stomach. The sensation of fulfillment from her tits is only eclipsed by the massive load of semen you feel cramping your prostate.");
        //[ (Herm-Dickgirl variant only)
        if (player.balls > 0) outputText(" Even your nuts are unbearably sore.");
        outputText(" As the milk begins to dry out of the Succubus' tit, you release it from your control and launch an impossible load of cum into the succubus. The demoness releases her hold of your cock and hops off your crotch and jumps to place her mouth over your erupting penis. Reflexively grabbing her head, you push your cock as deep as you can in her mouth and for minutes, pump stream after stream of hot lust into her gullet. After the last load leaves your dong, you pass out.<br><br>");

        outputText("After a short time, you wake up sore from head to toe. The Succubus is sitting next to you with an utterly satisfied look on her face.<br><br>");

        outputText("\"<i>Well, this was unexpected.</i>\", she says, \"<i>I did not expect you to change. Normally, men are susceptible to my milk, but apparently it works on herms, too.</i>\"<br><br>");
        ...
        outputText(" As the reality soaks in, you feel a sharp pain in your stomach and your cock. You NEED to feed. Cum, milk, it doesn't matter. Likewise, your dick is hard and you need to cum. Despite your need, you cannot bring yourself to masturbate. You want ANOTHER'S attention.<br><br>");
        ...
        outputText("<br>As you begin to relax from a long day of adventuring, the succubus returns and lands squarely in your lap, just missing your throbbing erection. The succubus growls in arousal as she thrusts one of her fat nipples into your mouth. Reflexively, you begin suckling the teat with neither shame nor restraint. Milk floods into your mouth as you sense the weight of the succubus descend upon your cock. The familiar warmth and snugness of her cunt greet your hungry prick as her muscles begin the savory churning to coax your body into producing the 'milk' she needs to sate her own hunger. Your eyes roll back into your head as the torrent of milk pouring down your throat increases the sensitivity in all of your organs, compelling your hips to reflexively buck to press your dick deeper.<br><br>");

        outputText("The Succubus restrains you without missing a stroke or disrupting your breastfeeding as the pangs of orgasmic pleasure swell up at the base of your cock. You wrap your arms forcefully around the succubus as you bear down upon your crotch, releasing the painfully stockpiled load of lust into the demoness' cunt for her own sustenance. The succubus lets out an inhuman howl of pleasure as her own orgasm begins to crush your cock, draining every last drop out of you.<br><br>");
